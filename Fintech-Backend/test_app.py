...
class TestAPI_MonkeyConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with self.app.app_context():
            ...
            admin = UserModel(CF="ADMINCF000000000", nome="First", cognome="Admin", email="admin@admin.com", is_admin=True)
    ...
    def get_auth_header(self, email, password):
        res = self.client.post("/api/v1/login/", json={"email": email, "password": password})
    ...
    def test_register_login_and_duplicate(self):
        # registrazione OK
        r1 = self.client.post("/api/v1/register/", json={
            "nome": "Alice", "cognome": "Rossi", "CF": "RSSALC85T10A562S",
            "email": "alice@example.com", "password": "Password1!"
        })
        self.assertEqual(r1.status_code, 201)
        # email duplicata
        r2 = self.client.post("/api/v1/register/", json={
        ...
        # login errato
        bad = self.client.post("/api/v1/login/", json={"email": "alice@example.com", "password": "Wrong1!"})
    ...
    def test_conto_crud_and_limits(self):
        ...
        hdr = self.alice_hdr
        # GET conti
        r = self.client.get("/api/v1/conti/", headers=hdr)
        self.assertEqual(r.status_code, 200)
        conti = r.get_json()
        self.assertEqual(len(conti), 1)
        # creo 4 altri conti
        for i in range(1, 5):
            rc = self.client.post("/api/v1/conti/", headers=hdr, json={"nome": f"C{i}"})
            self.assertEqual(rc.status_code, 201)
        # 6° conto -> 403
        r6 = self.client.post("/api/v1/conti/", headers=hdr, json={"nome": "X"})
        ...
        cid = conti[1]["id"]
        rp = self.client.patch(f"/api/v1/conto/{cid}/", headers=hdr, json={"nome": "Nuovo"})
        ...
        rd = self.client.delete(f"/api/v1/conto/{cid}/", headers=admin_hdr)
    ...
    def test_deposit_withdraw(self):
        self.test_register_login_and_duplicate()
        hdr = self.alice_hdr
        cid = self.client.get("/api/v1/conti/", headers=hdr).get_json()[0]["id"]
        # deposito
        d = self.client.patch("/api/v1/deposito/", headers=hdr, json={"id_conto": cid, "importo": 100.0})
        self.assertEqual(d.status_code, 201)
        # prelievo eccesso
        o = self.client.patch("/api/v1/prelievo/", headers=hdr, json={"id_conto": cid, "importo": 200.0})
        self.assertEqual(o.status_code, 403)
        # prelievo valido
        ok = self.client.patch("/api/v1/prelievo/", headers=hdr, json={"id_conto": cid, "importo": 50.0})
    ...
    def test_bonifico_and_movements(self):
        ...
        admin_hdr = self.get_auth_header("admin@admin.com", "Admin1234")
        idb_res = self.client.post("/api/v1/IDbyCF/", headers=admin_hdr, json={"CF": "RCTUSR85T10A562S"})
        self.assertEqual(idb_res.status_code, 200)
        recv_id = idb_res.get_json()["ID"]
        recv_conto = self.client.get(f"/api/v1/conti/{recv_id}/", headers=admin_hdr).get_json()[0]
        # fondo mittente
        alice_conto = self.client.get("/api/v1/conti/", headers=hdr).get_json()[0]
        self.client.patch("/api/v1/deposito/", headers=hdr, json={"id_conto": alice_conto["id"], "importo": 100.0})
        # bonifico
        b = self.client.post("/api/v1/bonifico/", headers=hdr, json={
        ...
        # movimenti mittente
        mv = self.client.get("/api/v1/movimenti/", headers=hdr).get_json()
        self.assertTrue(any(m["importo"] < 0 for m in mv))
        # movimenti destinatario
        rh_hdr = self.get_auth_header("recv@ex.com", "Password1!")
        mv2 = self.client.get("/api/v1/movimenti/", headers=rh_hdr).get_json()
        self.assertTrue(any(m["importo"] > 0 for m in mv2))

    def test_admin_endpoints(self):
        admin_hdr = self.get_auth_header("admin@admin.com", "Admin1234")
        # get non esistente
        r404 = self.client.get("/api/v1/admin/user/9999/", headers=admin_hdr)
        self.assertEqual(r404.status_code, 404)
        # register + get esistente
        self.test_register_login_and_duplicate()
        rg = self.client.get("/api/v1/admin/user/2/", headers=admin_hdr)
        self.assertEqual(rg.status_code, 200)
        # patch admin-user
        pg = self.client.patch("/api/v1/admin/user/2/", headers=admin_hdr, json={
            "nome": "X", "cognome": "Y", "CF": "NEWCF85T10A562S",
            "email": "new@example.com", "password": "Password1!"
        })
        self.assertEqual(pg.status_code, 200)
        # IDbyCF
        idb2 = self.client.post("/api/v1/IDbyCF/", headers=admin_hdr, json={"CF": "NEWCF85T10A562S"})
    ...
    def test_admin_movimenti_and_conto_movimenti(self):
        ...
        admin_hdr = self.get_auth_header("admin@admin.com", "Admin1234")
        # movimenti admin
        mva = self.client.get("/api/v1/admin/movimenti/2/", headers=admin_hdr)
        self.assertIn(mva.status_code, (200, 204))
        # movimenti conto specifico
        alice_conto = self.client.get("/api/v1/conti/", headers=self.alice_hdr).get_json()[0]
        mvc = self.client.get(f"/api/v1/conto/movimenti/{alice_conto['id']}/", headers=self.alice_hdr)
